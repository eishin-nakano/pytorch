...
class CppKernelProxy(CppKernel):
    ...
    def codegen_nodes(self, nodes: List[SchedulerNode]):
        ...
        def node_names(node) -> List[str]:
            return {n.get_name() for n in node.get_nodes() if isinstance(n, BaseSchedulerNode)}
        ...
        fn_list = [functools.partial(fn, node, fused_node_names=node_names(node)) for node in nodes]
